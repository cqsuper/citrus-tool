...
public class JavaHyperlinkDetector extends AbstractHyperlinkDetector {
	private final static Pattern	classPattern	= Pattern.compile("[\\w]+\\s?=\\s?[.$\\w]+");
	...
	public IHyperlink[] detectHyperlinks(ITextViewer textViewer, IRegion region, boolean canShowMultipleHyperlinks) {
		...
		while (matcher.find()) {
			...
			String className = line.substring(index).trim();
			return createJavaHyperlinks(document, className,
			        new Region(lineInfo.getOffset() + index, className.length()));
		}
